...
import numpy as np
import ollama
from src.config import EMBEDDING_CACHE_PATH, EMBEDDING_MODEL, TOP_N

logger = logging.getLogger(__name__)
# In-memory vector database: parallel lists kept in sync
_CHUNKS: list[str] = []
_EMBEDDINGS: np.ndarray | None = None  # shape (n_chunks, embedding_dim)
...
def _embed(text: str) -> list[float]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not reach Ollama to embed text using model '{EMBEDDING_MODEL}'. "
            "Is the Ollama server running, and has the model been pulled "
            f"(`ollama pull {EMBEDDING_MODEL}`)? Original error: {exc}"
        ) from exc
    return response["embeddings"][0]

def _load_cache(fingerprint: str) -> tuple[list[str], np.ndarray] | None:
    # Load cached embeddings if the fingerprint matches, otherwise return None
    ...
    logger.info("Loaded %d cached embeddings from %s", len(chunks), EMBEDDING_CACHE_PATH)
    return chunks, embeddings

def _save_cache(fingerprint: str, chunks: list[str], embeddings: np.ndarray) -> None:
    # Save the current embeddings to the cache for future runs
    ...
    with open(EMBEDDING_CACHE_PATH, "w", encoding="utf-8") as f:
        json.dump(payload, f)
    logger.info("Cached %d embeddings to %s", len(chunks), EMBEDDING_CACHE_PATH)
